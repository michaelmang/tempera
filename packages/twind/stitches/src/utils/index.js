...
export function getClassName(property, value, theme) {
  ...
  const [__, nameClass] =
    Object.entries(nameClasses).find(([tailwindPlugin]) => {
  ...
  const [key] =
    Object.entries(tokens).find(([_, tailwindValue]) => {
      if (
        Array.isArray(tailwindValue) &&
        tailwindValue.every((x) => typeof x === "string")
      ) {
      ...
      if (
        Array.isArray(tailwindValue) &&
        tailwindValue.some((x) => typeof x === "object")
      ) {
